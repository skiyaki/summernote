...
var ImageDialog_ImageDialog = /*#__PURE__*/function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group note-group-select-from-files">', '<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="' + this.options.acceptImageFileTypes + '" />', imageLimitation, '</div>', '<div class="form-group note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url form-control note-form-control note-input" type="text"/>', '</div>'].join('');
...
var VideoDialog_VideoDialog = /*#__PURE__*/function () {
  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function initialize() {
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group row-fluid">', "<label for=\"note-dialog-video-url-".concat(this.options.id, "\" class=\"note-form-label\">").concat(this.lang.video.url, " <small class=\"text-muted\">").concat(this.lang.video.providers, "</small></label>"), "<input id=\"note-dialog-video-url-".concat(this.options.id, "\" class=\"note-video-url form-control note-form-control note-input\" type=\"text\"/>"), "<a class=\"note-video-link\" target=\"_blank\" href=\"/club/dashboard/movies\">".concat(this.lang.video.upload, "</a>"), '</div>'].join('');
    ...
    value: function createVideoNode(url) {
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe allowfullscreen>').attr('frameborder', 0).attr('allow', 'accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture').attr('src', '//www.youtube.com/embed/' + youtubeId + (start > 0 ? '?start=' + start : '')).attr('width', '100%').attr('height', '360');
      } else if (gdMatch && gdMatch[0].length) {
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe allowfullscreen>').attr('frameborder', 0).attr('allow', 'accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture').attr('src', 'https://drive.google.com/file/d/' + gdMatch[1] + '/preview').attr('width', '100%').attr('height', '480');
      } else if (igMatch && igMatch[0].length) {
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe allowfullscreen>').attr('frameborder', 0).attr('allow', 'accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture').attr('src', 'https://instagram.com/p/' + igMatch[1] + '/embed/').attr('width', '100%').attr('height', '360').attr('scrolling', 'no').attr('allowtransparency', 'true');
      } else if (vimMatch && vimMatch[3].length) {
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>').attr('frameborder', 0).attr('allow', 'accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture').attr('src', '//player.vimeo.com/video/' + vimMatch[3]).attr('width', '100%').attr('height', '360');
      } else {
    ...
    value: function show() {
      ...
      this.showVideoDialog(text).then(function (url) {
        ...
        if ($node) {
          ...
          _this.context.invoke('editor.insertNode', external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<p><br></p>')[0]);
        }
      }).fail(function () {
        _this.context.invoke('editor.restoreRange');
      });
